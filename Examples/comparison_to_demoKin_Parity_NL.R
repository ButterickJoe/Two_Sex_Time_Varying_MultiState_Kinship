...
for(year in 1965:1966){
  ...
  foreach(stage = 1:ns)%do%{
    mort_rates_fem[,stage] <- c(Matrix::diag(U_list_F[[indx_year]][-1,-ncol(U_list_F[[indx_year]])]), U_list_F[[indx_year]][na,na])
    mort_rates_male[,stage] <- c(Matrix::diag(U_list_M[[indx_year]][-1,-ncol(U_list_M[[indx_year]])]), U_list_M[[indx_year]][na,na])
  }
...
kin_out_1965_NL <- 
  kin_multi_stage_TV_2_sex_tandem_NL(U_mat_fem[1:1], ## 40 years time-series of demographic rates (1965-2005) see last input...
                         U_mat_male[1:1],
                         F_mat_fem[1:1],
                         F_mat_male[1:1],
                         T_mat_fem[1:1],
...
kin_out_1965_NL$group%>%unique()

kin_out_1965_NL %>% dplyr::filter(group == "parents", Sex == "Female", year == 1965) %>%
  dplyr::select(Age_Focal,Stage,pred_no_kin) %>%
  ggplot2::ggplot(aes(x = Age_Focal, y = pred_no_kin, color = Stage, fill = Stage)) +
  geom_bar(position = "stack", stat = "identity")

kin_out_1965_NL %>% dplyr::filter(group == "Focal", Sex == "Female", year == 1965) %>%
  dplyr::select(Age_Focal,Stage,pred_no_kin) %>%
  ggplot2::ggplot(aes(x = Age_Focal, y = pred_no_kin, color = Stage, fill = Stage)) +
  geom_bar(position = "stack", stat = "identity")


df_export <- kin_out_1965_NL %>% 
  dplyr::filter(Sex == "Female", year == 1965) %>%
  dplyr::select(Age_Focal,Stage,pred_no_kin,group) %>%
  dplyr::transmute(stage_kin = Stage,
                   age_focal = Age_Focal,
                   living = pred_no_kin, 
                   kin = group)
...
saveRDS(df_export,"C:/Users/jb4u23/OneDrive - University of Southampton/Butterick year 1/R code/First files/Implementing Caswell/DemoKin/DemoKin-main/Butterick_parity_comparison/Joes_mother_parity_NL.Rds")
