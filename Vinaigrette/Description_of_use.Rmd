...
#options(dplyr.summarise.inform = FALSE) hide if we don't want to see summarise output (but also loose progress bar)

```

In this vignette, we'll demonstrate how `Two_Sex_Time_Varying_MultiState_Kinship` can be used to compute parity-specific kinship networks for an average member of a given population, the sex of whom is user specified, subject to time-varying demographic rates. We call this individual Focal. We seek the number, age, and parity distribution of Focal's relatives for each age of Focal's life as a function of the year in which Focal is born. 

## Kin counts by parity ##########################################################################

In this example we use data for the UK ranging from 1965 - 2005, sourced from the [Human Mortality Database](https://www.mortality.org/) and [Human Fertility Database](https://www.humanfertility.org/). Some simplifying assumptions we make due to current data availability are as follows:
...
kin_out_1965_1985 <- 
  kin_multi_stage_TV_2_sex(U_mat_fem[1:(1+no_years)],
                           ...
                           seq(1965, (1965 + no_years)) ## the sequence of years we run the function over
  )
```
...
kin_out_1965_1985 %>% 
  ...
  ggplot2::ylab("Offspring") 
```
The LHS plot (1900 cohort) should be interpreted as follows: if Focal is born in 1900, between 1965-1985 he/she will be 65-85 years old; offspring will be around 30-45, have completed reproduction/parity progression, and will be experiencing mortality.
