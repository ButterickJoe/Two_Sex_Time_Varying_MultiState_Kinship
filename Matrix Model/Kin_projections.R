...
all_kin_dy <- function(Uf, 
  ...
  ## Uproj is a block diagonal matrix of block-structured Age*Stage matrices
  ## For females and males independently it transfers individuals across stage and up age
  
  Uproj <- as(block_diag_function(list(Uf, Um)),"sparseMatrix") ## ( block_diag_func in "Functions_required.R" )
  
  ## Fproj is a Sex-block-structured matrix of block-structured Age*Stage matrices where males and females reproduce (by stage)
  ...
  ## Fprojstar is a Sex-block-structured matrix of block-structured Age*Stage matrices where only females reproduce (by stage)
  ...
  # Age*Stage range (defines number of rows of kin output)
  ...
  ## Initial conditions (female and male granparents)
  IC_f_grand_pars <- mothers_age_dist
  IC_m_grand_pars <- fathers_age_dist
  IC_gps <- rep(0, 2*n)
  parents <- X_parents
  foreach(ic = 1 : (na))%do%{
    IC_gps <- IC_gps + (IC_f_grand_pars[ic] + IC_m_grand_pars[ic])*parents[,ic]
  }
  ...
  ## IC
  IC_younger_sibs <- rep(0, n*2) ## No younger sisters at birth IC = (0,0,....,0) (2n length)
  ...
  ## At birth, older Nieces/Nephews given by grandchildren of mom :
  ## IC given by expected number of grandchildren of mother * age specific fertility
  IC_onn <- rep(0, n*2) 
